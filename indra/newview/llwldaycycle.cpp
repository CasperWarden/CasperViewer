...
void LLWLDayCycle::loadDayCycle(const LLSD& day_data, LLWLParamKey::EScope scope)
{
	...
	for(S32 i = 0; i < day_data.size(); ++i)
	{
		...
		success =
			LLWLParamManager::instance()->getParamSet(frame, pset);
		if(!success)
		{
			...
			if (scope == LLEnvKey::SCOPE_REGION)
			{
				...
				success = LLWLParamManager::instance()->getParamSet(frame, pset);
			}
...
bool LLWLDayCycle::changeKeyframeParam(F32 time, LLWLParamKey key)
{
	...
	LLWLParamSet tmp;
	bool stat = LLWLParamManager::instance()->getParamSet(key, tmp);
	if(stat == false) 
	{
